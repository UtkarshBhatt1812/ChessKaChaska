...
export type PlayerInfo = {
  ...
  color: PlayerColor;
  connected: boolean;
  disconnectedAt?: number; // epoch ms, for grace-period logic
};
...
export type TimeControl = {
  initialSeconds: number; // e.g. 600 for 10 min
  incrementSeconds: number;
  mode: "bullet" | "blitz" | "rapid" | "classical" | "custom";
};

export type CapturedPieces = {
  white: string[]; // pieces captured BY white (black pieces lost)
  black: string[];
};
...
export type GameState = {
  ...
  timers: { white: number; black: number }; // remaining ms
  isCheck: boolean;
  isCheckmate: boolean;
  isStalemate: boolean;
  isDraw: boolean;
...
export type CreateRoomPayload = {
  timeControl: TimeControl;
  color?: PlayerColor; // preferred color; server may assign
};
...
export type ServerToClientEvents = {
  ...
  resign: (payload: { color: PlayerColor }) => void;
  draw_offer: (payload: { fromColor: PlayerColor }) => void;
  player_disconnected: (payload: { color: PlayerColor; username: string }) => void;
  player_reconnected: (payload: { color: PlayerColor; username: string }) => void;
